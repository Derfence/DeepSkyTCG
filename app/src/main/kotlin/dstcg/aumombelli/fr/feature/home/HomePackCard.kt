...
internal fun HomePackCard(
    ...
    MotionCard(
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            HomePackCardBackground(modifier = Modifier.fillMaxSize())
...
@Composable
private fun HomePackCardBackground(
    modifier: Modifier = Modifier,
) {
    val context = LocalContext.current.applicationContext
    val svgMarkup = remember(context) {
        runCatching {
            context.assets.open(HomePackCardSvgAssetName).bufferedReader().use { reader ->
                reader.readText()
            }
        }.getOrNull()
    }
    ...
    if (svgMarkup != null) {
        HomePackCardSvgBackground(
            svgMarkup = svgMarkup,
            modifier = modifier,
        )
    } else {
        LegacyHomePackCardBackground(modifier = modifier)
    }
}

@SuppressLint("SetJavaScriptEnabled")
@Composable
private fun HomePackCardSvgBackground(
    svgMarkup: String,
    modifier: Modifier = Modifier,
) {
